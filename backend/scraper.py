...
def parse_cse_faculty(soup):
    ...
    # -------- Case 1: The 'wp-block-cover' pattern (e.g., Dr. Venkatesh) --------
    # This pattern has a cover image followed by a separate h2/h4 and p.
    for cover in soup.find_all("div", class_="wp-block-cover"):
        img_tag = cover.find("img")
        image = resolve_url(img_tag["src"]) if img_tag and img_tag.get("src") else None

        heading = cover.find_next_sibling(lambda tag: tag and tag.name in ["h2", "h4"])
        p = heading.find_next_sibling("p") if heading else None
        ...
        link_tag = heading.find_next_sibling("ul")
        if link_tag:
            a = link_tag.find("a", href=True)
            if a:
                profile = resolve_url(a["href"])
    ...
    # This pattern has a figure, followed by an h2, and then a p.
    for fig in soup.find_all("figure", class_="wp-block-image"):
        heading = fig.find_next_sibling(lambda tag: tag and tag.name in ["h2", "h4"])
        if heading: # This is the key condition for this pattern
            img_tag = fig.find("img")
            image = resolve_url(img_tag["src"]) if img_tag and img_tag.get("src") else None
    ...
    for cover in soup.find_all("div", class_="wp-block-cover"):
        ...
        # -------- Case 5: h2 → figure → p (pattern for Prof. Shree Gowri S S) --------
    for h2 in soup.find_all("h2", class_="wp-block-heading"):
        name = h2.get_text(strip=True)
        if not name:
            continue

        # Check if next element is a figure
        fig = h2.find_next_sibling("figure", class_="wp-block-image")
        if not fig:
            continue
        ...
        # Next <p> contains designation + email
        p = fig.find_next_sibling("p")
        if p:
            text = p.get_text(" ", strip=True)
            email_match = re.search(r"[\w\.-]+@[\w\.-]+", text)
            email = email_match.group(0) if email_match else None
    ...
    # -------- Force order: Venkatesh first, Apoorva second --------
    priority_order = [
        "dr. venkatesh s.",
        "ms. farzana n. nadaf",
        "mrs. shree gowri s s",
        "mr. pranesh kulkarni",
        "mr. girish shivanagudi",
        "mr. bhagat inamdar",
        "mrs. sunita kallu",
        "mrs. nirmala ganiger",
        "mrs. apoorva sabnis",
    ...
    def rank(f):
        ...
        for i, expected in enumerate(priority_order):
            if expected in name:
                return i
