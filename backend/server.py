...
def parse_cse_faculty(soup):
    ...
    def add_faculty(name, designation=None, email=None, phone=None, image=None, profile=None):
        if not name:
            return
    ...
    # 1) figure + figcaption entries (existing working case)
    for fig in soup.find_all("figure", class_="wp-block-image"):
        ...
        if figcaption:
            ...
            if name:
                add_faculty(name=name, email=email, image=image)

    # 2) h2 / h4 headings that contain name + designation + email/phone (Venkatesh/Apoorva style)
    for heading in soup.find_all(["h2", "h4"]):
        text = heading.get_text(" ", strip=True)
        if not text:
            continue

        if not re.search(r"\b(Dr|Mr|Mrs|Ms|Prof)\b", text, re.IGNORECASE):
            continue
        ...
        # break heading text into lines to separate name + designation information
        lines = [line.strip() for line in re.split(r"\r?\n|<br\s*/?>", text) if line.strip()]
        name = lines[0] if lines else text
        name = re.sub(r"[\w\.-]+@[\w\.-]+", "", name).strip()
        name = re.sub(r"\b\d{10}\b", "", name).strip()

        designation = ""
        if len(lines) > 1:
            designation = " ".join(lines[1:]).strip()

        img_tag = heading.find_previous("img") or heading.find_next("img")
        image = resolve_url(img_tag["src"]) if img_tag and img_tag.get("src") else None

        if name:
            add_faculty(name=name, designation=designation, email=email, phone=phone, image=image)
...
def add_notice():
    ...
    try:
        ...
        if category == "placement" and not image_url:
            return jsonify({"success": False, "error": "Placement notices requires an image."}), 400
